...
//By: David Gilson
//File for creating DTLB functions
#include "DTLB.h"

//Config Setup for DTLB
DTLB::DTLB(int sn, int ssz, int dtlb_index_bits_num, int offset_bits_num)
{
    set_num = sn;
    set_size = ssz;
    bits_index = dtlb_index_bits_num;
    bits_offset = offset_bits_num;
...
//To see if the cache has the specified entry
int DTLB::check_match(int tag, int index)
{
    int entry = -1;
	//Search set, check valid and tag
...
//Function to evict LRU entry from specified set. Returns which entry in the set was evicted
int DTLB::LRU_Evict(int index)
{
    int lineEvicted = -1;     
	//If direct mapped
    if(_type == 0)                                         
    ...
    else if (_type == 1)                                   
    {
        ...
        lineEvicted = current_LRU_Entry;
    }
    return lineEvicted;                                                            
}

//Function that allocates a line in the cache. Will return entry in set allocated into
int DTLB::allocate(std::string addr, int tag, int index, int VPN, int PFN)
{
    int line;                                                        
    ...
    if(!foundFreeEntry)
    {
        int entry = LRU_Evict(index);                       

        cache.at(index).at(entry).index = index;                     
        cache.at(index).at(entry).tag = tag;                         
        cache.at(index).at(entry).valid = true;                      
        cache.at(index).at(entry).reference_counter = 1;             
        cache.at(index).at(entry).addr = addr;                       
        cache.at(index).at(entry).VPN = VPN;                         
        cache.at(index).at(entry).PFN = PFN;                         
        line = entry;
    }
    return line;                                                     
}

//Function that returns a vector of ints given the address. Return Vector: Position 0 is the tag and position 1 is the index
std::vector<int> DTLB::getAddrTagIndex(std::string addr)
{
...
void DTLB::incRefCounter(int tag, int index)
{
    for(int i = 0; i < set_size; i++)
    {
        if(tag == cache.at(index).at(i).tag && cache.at(index).at(i).valid)        
...
//Function to get PFN from the entry. This method will only be called if line is valid
int DTLB::retrievePFN(int tag, int index)
{
    int PFN = -1;
    for(int i = 0; i < set_size; i++)
    {
...
//Function to invalidate cache entries associated with a specific PFN (used with main memory)
void DTLB::invalidateByPFN(int PFN)
{
    for(int i = 0; i < set_num; i++)
    {
